...
cad.defineShape("heart",function(paper,cx,cy,option){
    ...
    for(var i = 0; i < num; i ++) {
        ...
        point.x = point.x*option.size/32.69;
        point.y = point.y*option.size/32.69;
        points.push(point);
    }
    var path = new cad.Path();
...
cad.defineShape("sinLine",function(paper,cx,cy,option){
    var path = new cad.Path();
    var ret = cad.rangeInterpolate("sin",-10,10,60);
    var x = ret[0], y = ret[1];
    var scale = 20;
...
cad.defineShape("bendLine",function(paper,cx,cy,option){
    ...
    var  h  = option.h;
    var path = new cad.Path();
    var points = cad.rangeInterpolate("bend",0,1,40);
    var max = cad.interpolate("bend",0.5);
    var px =points[0],
        py = points[1];
    var ret = px.map(function(val,index){
        var x = cx - len/2 + len*val;
        var y = py[index]/max*h + cy;
        return {x,y};
    })
    path = path.LineToPoints(ret).toString();
    return paper.append("path").attr("d",path);
})