...
Paper.prototype = {
    ...
    initPaper:function(){
        ...
        var default_layer = this.createSVGElement('g',{id:"default_layer",stroke:"#000",fill:"none"});
        $(el).append(svg);
        $(svg).append(default_layer);
        this.doc = svg;
    },
    append:function(tagName,option){
        var el = this.createSVGElement(tagName,option);
        var selector = "#" + (this.currentLayer||"default_layer");
        var layer = this.select(selector);
        layer.append(el);
        return $(el);
    },
