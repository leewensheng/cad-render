...
Pie.prototype = {
	...
	selectSector:function(val,self){
		...
		if(!this.text) {
			this.text = this.paper.text(angle,cx,cy,{
				fontSize:100,
				align:"center",
				baseline:"middle",
				color:"red"
	...
	attachEvents:function(){
		...
		sectors.map(function(val,index){
			val.el.on("click",function(){
				...
				var color = val.color;
				var hoverColor = cad.darken(color,0.3);
				val.el.transition({fill:hoverColor},400,'easeOut');
			}).on("mouseout",function(){
				val.el.transition({fill:val.color},400,'easeOut');
			})
		})
	},
